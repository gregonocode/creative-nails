...
/** =========================================================
 *  ✅ UTM / tracking persistence (quiz -> oferta)
 *  ========================================================= */
...
function pickTrackingParamsFromSearch(search: string) {
  ...
  for (const [k, v] of sp.entries()) {
    ...
    if (key === "fbclid" || key === "fbc" || key === "fbp") out.set(k, v);
  }
...
export default function QuizFlow() {
  ...
  // ✅ salva tracking assim que a pessoa entra no quiz
  useEffect(() => {
    const incoming = pickTrackingParamsFromSearch(window.location.search);
      ...
      <div className="mx-auto w-full max-w-[780px] px-4 py-10 sm:py-14">
        ...
        <div className="mb-6 flex items-center justify-between">
          <div className="inline-flex items-center gap-2">
            ...
            <p className="text-sm font-medium text-slate-700">Quiz rápido • 20 segundos</p>
          </div>
            ...
            <>
              <div className="relative overflow-hidden rounded-3xl border border-sky-100 bg-white p-5 shadow-sm sm:p-7">
                ...
                <p className="relative mt-2 text-sm text-slate-600">
                  Separando os modelos 3D mais alinhados com seu objetivo
                  {name.trim() ? `, ${name.trim()}` : ""}.
                </p>
        ...
        {/* Footer note */}
        <p className="mt-6 text-center text-xs text-slate-500">Kitnetes de • baixo custo</p>
      </div>
    </div>
  );
}